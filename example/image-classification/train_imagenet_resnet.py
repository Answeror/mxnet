...
# network
from symbol_resnet_v2 import get_symbol
net = get_symbol(num_class=1000, num_level=5, num_block=9, num_filter=16, pool_kernel=(4, 4))
import memonger
net = memonger.search_plan(net, data=(args.batch_size, 3, 128, 128))
# num_block=9 5691MiB
    ...
    def __call__(self, num_update):
        ...
        if self.cur_step_ind <= len(self.step)-1:
            if num_update > self.step[self.cur_step_ind]:
                ...
                self.cur_step_ind += 1
                self.base_lr *= self.factor[self.cur_step_ind - 1]
...
def fit(args, network, data_loader, batch_end_callback=None):
    ...
    model = mx.model.FeedForward(
        ...
        lr_scheduler=Scheduler(
            epoch_step=[s if args.load_epoch is None else s - args.load_epoch
                        for s in [30, 60]],
            factor=[0.1, 0.1],
