...
def get_conv(
    ...
    conv = lambda data: mx.symbol.Convolution(
        name=name,
        data=data,
    ...
    bn = lambda data: mx.symbol.BatchNorm(
        name=name + '_bn',
        data=data,
        fix_gamma=False,
    ...
    relu = lambda data: mx.symbol.Activation(
        name=name + '_relu',
        data=data,
        act_type='relu'
    )
    net = conv(relu(bn(data))) if preact else relu(bn(conv(data)))
...
def get_symbol(
    ...
    body = mx.symbol.Activation(name='relu', data=body, act_type='relu')
    ...
    pool = mx.symbol.Pooling(data=relu, kernel=pool_kernel, pool_type='avg')
    fc = mx.symbol.FullyConnected(data=pool, num_hidden=num_class, name='fc')
